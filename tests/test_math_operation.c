...
START_TEST(math_operation_test_05) {
  ...
  add_data_to_stack_ch(sc, '/');
  return_value = math_operation(&sn, &sc);
  ck_assert_int_eq(return_value, CALC_ERR);
  ck_assert_double_eq_tol(sn->data, 3.3, 0.7);
  free_stack_num(sn);
  free_stack_ch(sc);
}
...
START_TEST(math_operation_test_08) {
  Stack_num *sn = NULL;
  Stack_ch *sc = NULL;
  int return_value = 0;
  sn = push_stack_num(sn);
  add_data_to_stack_num(sn, 100);
  sc = push_stack_ch(sc);
  add_data_to_stack_ch(sc, '%');
  sn = push_stack_num(sn);
  add_data_to_stack_num(sn, 11);
  return_value = math_operation(&sn, &sc);
  ck_assert_int_eq(return_value, OK);
  ck_assert_double_eq_tol(sn->data, 1, 0.7);
  free_stack_num(sn);
  free_stack_ch(sc);
}
END_TEST

START_TEST(math_operation_test_09) {
  Stack_num *sn = NULL;
  Stack_ch *sc = NULL;
  int return_value = 0;
  sc = push_stack_ch(sc);
  add_data_to_stack_ch(sc, TAN);
  sn = push_stack_num(sn);
  add_data_to_stack_num(sn, 11);
  return_value = math_operation(&sn, &sc);
  ck_assert_int_eq(return_value, OK);
  ck_assert_double_eq_tol(sn->data, -225.950846, 0.7);
  free_stack_num(sn);
  free_stack_ch(sc);
}
END_TEST
...
Suite *test_math_operation() {
  ...
  tcase_add_test(tc_math_operation, math_operation_test_08);
  tcase_add_test(tc_math_operation, math_operation_test_09);
