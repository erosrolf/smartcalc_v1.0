...
START_TEST(token_parsing_test_01) {
  ...
  return_value = token_parsing(str, &sn, &sc, &ch_pointer, &is_unary, 0);
...
START_TEST(token_parsing_test_04) {
  ...
  while (str[ch_pointer]) {
    token_parsing(str, &sn, &sc, &ch_pointer, &is_unary, 0);
  }
  ck_assert_int_eq(sc->data, '-');
  ck_assert_int_eq(sc->next->data, '*');
  ck_assert_double_eq_tol(sn->next->next->data, -12345.6789, 1e-06);
  ck_assert_double_eq_tol(sn->next->data, 0.000004, 1e-06);
...
START_TEST(token_parsing_test_09) {
  Stack_num *sn = NULL;
  Stack_ch *sc = NULL;
  int is_unary = 0;
  unsigned int ch_pointer = 0;
  char str[] = "sqrt(X)";
  while (str[ch_pointer]) {
    token_parsing(str, &sn, &sc, &ch_pointer, &is_unary, 12345);
  }
  ck_assert_int_eq(sc->next->next->data, SQRT);
  ck_assert_int_eq(sc->next->data, '(');
  ck_assert_int_eq(sc->data, ')');
  ck_assert_float_eq_tol(sn->data, 12345, 1e-06);
  free_stack_num(sn);
  free_stack_ch(sc);
}
END_TEST

START_TEST(token_parsing_test_10) {
  Stack_num *sn = NULL;
  Stack_ch *sc = NULL;
  int is_unary = 1;
  unsigned int ch_pointer = 0;
...
Suite *test_token_parsing() {
  ...
  tcase_add_test(tc_token_parsing, token_parsing_test_09);
  tcase_add_test(tc_token_parsing, token_parsing_test_10);
